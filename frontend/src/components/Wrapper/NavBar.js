...
const NavBar = () => {
  return (
    <Main>
      ...
      {NavbarLinks.map((i)=><NavLink key={i.text} className={({ isActive }) => (isActive ? 'active' : '')} to={i.link}>{i.text}</NavLink>)}
      <NavLink className={({ isActive }) => (isActive ? 'active' : '')} to="/register">login</NavLink>
      </div>
    </Main>
  )
}
